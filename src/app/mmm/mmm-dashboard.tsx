...
// Capped at 12 (not 24): like scope-13, TAG-IT's sorted "new shape" projection
// scales ~2s/doc, so size 24 + sort=-meta.document_date risks the 50s timeout
// (502). size 12 completes comfortably. Pending a TAG-IT-side sort index.
const PAGE_SIZE = 12;
  ...
  const fetchData = useCallback(
    async (
      ...
      try {
        ...
        const res = await fetch(`/api/mmm/documents?${qs}`);
        if (!res.ok) {
    ...
    <div dir="rtl">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
        ...
        <div className="mb-3">
          ...
          >
            חיפוש חופשי בתוך המסמכים
          </label>
          <input
            id="mmm-q"
            type="text"
            placeholder="חיפוש בכותרת ובתוכן מסמכי מרכז המחקר והמידע"
            value={draft.q}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label htmlFor="mmm-date-from" className="block text-xs font-semibold text-gray-700 mb-1">
              מתאריך
            </label>
            <DateInputIL
              id="mmm-date-from"
          ...
          <div>
            <label htmlFor="mmm-date-to" className="block text-xs font-semibold text-gray-700 mb-1">
              עד תאריך
            </label>
            <DateInputIL
              id="mmm-date-to"
        ...
        {/* Type facet pills — derived from current results, applied instantly. */}
          ...
          {loading ? (
            ...
            <span>
              {total === 0
                ...
                : `מציג מסמכים ${pageStart}–${pageEnd} מתוך ${total}`}
            </span>
          )}
        </div>
        {extraSortFields.length > 0 ? (
          <div className="flex items-center gap-2 shrink-0">
            <label htmlFor="mmm-1" className="text-xs text-gray-600">סדר:</label>
            <select id="mmm-1"
      ...
      <p className="text-xs text-gray-700 mt-8 leading-relaxed">
        המידע נשאב ממאגר מסמכי מרכז המחקר והמידע של הכנסת ומוצג כמות שהוא.
        הקישורים מובילים לקובץ ה-PDF המקורי כפי שפורסם.
      </p>
    </div>
  );
}
