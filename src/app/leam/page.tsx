...
import type { LeamPageContent, LeamSiteItem } from "@/types/content";
...
/* ─── Icon resolver ───────────────────────────────────────────────────
 *
 * Icons referenced by name from CMS-stored content. Falls back to Code2
 * if an unknown name slips through so the page never crashes.
 */
...
// it down as a prop. We can't do `const Icon = resolveIcon(site.icon)`
// inside this body — the react-hooks/static-components lint rule reads that
// PascalCase binding as a new component definition and (incorrectly here)
// flags it. Destructuring the already-resolved component from props sidesteps
// the heuristic.
      ...
      <div className="relative z-10 flex flex-col gap-5 p-6 sm:p-8">
        ...
        <div className="pt-1">
          <Link
            ...
            aria-label={`${ctaLabel} ${site.name} — נפתח בכרטיסייה חדשה`}
          >
            <span>{ctaLabel}</span>
...
export default async function LeamPage() {
  const content = await getPageContent<LeamPageContent>("leam");
  const editHref = "/admin/site-editor/leam";
  ...
  return (
    <PublicLayout>
      ...
      {/* ── Sites grid ──────────────────────────────────────────────── */}
      <EditableSection editHref={editHref} editLabel="האתרים">
        <section
