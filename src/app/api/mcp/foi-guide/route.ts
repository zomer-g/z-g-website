...
const TOOLS = [
  {
    ...
    description:
      ...
      "כל התאמה מחזירה: chapterUrl, snippet (כלל משפטי מהפרק), " +
      "**citedInSnippet** (פסקי דין שמבססים את הכלל — היחיד שמותר לצטט), " +
      "otherInChapter (להקשר בלבד, **אסור לצטט**). " +
      "**מבנה התשובה הנדרש**: לכל מבחן בנפרד — (א) ניסוח המבחן מה-snippet, " +
      "(ב) ציטוט פסק הדין מ-citedInSnippet (שם תיק + צדדים + תאריך, ללא " +
      "מספרי הערות שוליים), (ג) יישום לעובדות. חזור על המבנה למבחן הבא. " +
...
function renderResultsMarkdown(
  ...
  lines.push("");
  lines.push("> ⚠️ **כלל מחייב לתשובה לפי תוצאות אלה:**");
  lines.push(
    "> כל ציטוט של פסק דין בתשובה חייב להיות מתוך רשימת " +
      "**caseLaw** של אחד הפרקים למטה — מילה במילה. " +
      "**אסור** לצטט תיקים שאינם מופיעים שם, גם אם הם נראים רלוונטיים. " +
      "אם נושא ספציפי לא מכוסה ע\"י הציטוטים שבחיפוש — אמור זאת " +
      'מפורשות ("הכלל הזה מופיע במדריך אך לא הוחזר עם פסיקה תומכת בחיפוש"), ' +
      "אל תמלא את החסר ממקור אחר.");
  lines.push("");
    ...
    case "initialize":
      return rpcOk(req.id, {
        ...
        instructions:
          ...
          "\nהמשתמש מצפה לניתוח משפטי בסגנון של עתירה — לא לסיכום ופחות לא " +
          "ל-bullet-points. המבנה הוא:" +
          "\n\n1. **זיהוי הסוגיה המשפטית** (פסקה אחת): מהי הטענה/החלטה " +
          "שעומדת לבחינה (למשל: 'דחיית בקשת מידע בעילה של סעיף 9(ב)(4) " +
          "לחוק — חוות דעת/דיונים פנימיים')." +
          "\n\n2. **הצגת המבחנים** — כל מבחן בפסקה נפרדת, בסדר הזה:" +
          "\n   א. ניסוח המבחן (מתוך snippet — הכלל המשפטי כלשונו במדריך)." +
          "\n   ב. ציטוט פסק הדין שמבסס את המבחן (מתוך citedInSnippet): שם " +
          "התיק (סוג + מספר), שם הצדדים, ותאריך — למשל \"בעניין עע\"מ 10845/06 " +
          '**שידורי קשת בע"מ נ\' הרשות השנייה לטלוויזיה ורדיו** (11.11.2008) ' +
          'קבע בית המשפט העליון כי...".' +
          "\n   ג. **יישום** המבחן לעובדות המקרה הספציפי שהוצג." +
          "\n\n3. **חזור על המבנה (א-ב-ג)** לכל מבחן רלוונטי בנפרד. אל תכניס " +
          "כמה מבחנים לפסקה אחת. כל מבחן עומד בפני עצמו." +
          "\n\n4. **דוגמה מנחה**: סעיפים 25-47 ב-עת\"מ 25-05-_____ זומר נ' " +
          "מרכז למיפוי ישראל מציגים בדיוק את המבנה הזה ביחס לסעיף 9(ב)(6) — " +
          "טענה → דחייה → מבחן 1 + ציטוט + יישום → מבחן 2 + ציטוט + יישום וכו'." +
          "\n\n## חוקי ציטוט (חובה)" +
          "\n• **כל ציטוט פסיקה חייב להגיע מ-citedInSnippet של אותה תוצאה.** " +
          "אסור לקחת מ-citedInSnippet של תוצאה X להוכחת כלל מתוצאה Y. אסור " +
          "לקחת מ-otherInChapter בכלל. **אסור להמציא ציטוטים — גם לא כאלה " +
          "שנשמעים סבירים לפי הזיכרון.**" +
          "\n• פורמט: סוג ההליך + מספר התיק + **שם הצדדים** (מודגש) + תאריך " +
          "בסוגריים — בדיוק כפי שמופיע ב-text של citedInSnippet." +
          "\n• **אסור** לרשום מספרי הערות שוליים ([32]/[33א]) בתשובה." +
          "\n• הקישור (url) — data בלבד. צרף רק אם המשתמש ביקש קישור." +
          "\n• אם citedInSnippet ריק עבור כלל מסוים — נסח את הכלל מהטקסט, " +
          "אבל אמור מפורשות 'לא הוחזר פסק דין תומך ב-snippet. הציטוט הספציפי " +
          "במדריך הוא הערת שוליים [N] שלא הופיעה בקטע שאוחזר — אם נדרש " +
          "ציטוט, יש לחפש שוב את אותה הערה.' **אל תמציא.**" +
          "\n• אם הכלי החזיר 0 תוצאות לשאילתה, נסה ניסוח אחר לפני שתפנה ל-web." +
          "\n• **חובה** לציין בסוף התשובה קישור לפרק/ים במדריך (chapterUrl).",
      });
