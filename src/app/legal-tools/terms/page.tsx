...
async function getContent() {
  try {
    const page = await prisma.page.findUnique({ where: { slug: "legal-tools-terms" }, select: { content: true } });
...
export default async function LegalToolsTermsPage() {
  ...
  return (
    <PublicLayout>
      <section className="bg-primary py-16 sm:py-20 text-center">
        <Container>
          <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight">תנאי שימוש — כלים משפטיים</h1>
          <p className="mt-4 text-lg text-white/80">Terms of Service — Legal Tools</p>
        </Container>
      </section>
