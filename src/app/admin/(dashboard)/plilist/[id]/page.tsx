...
export default function EditPlilistPostPage({
  params,
}: {
  ...
  useEffect(() => {
    async function fetchPost() {
      try {
        const res = await fetch(`/api/plilist/${id}`);
        if (!res.ok) throw new Error("שגיאה בטעינת הפוסט");

        const post: PlilistPost = await res.json();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.error ?? "שגיאה בעדכון הפוסט. נסו שוב.");
      }
  ...
  async function handleDelete() {
    const confirmed = window.confirm(
      `למחוק את הפוסט "${title}"? פעולה זו אינה הפיכה.`,
    );
    if (!confirmed) return;
  ...
  /* ── Preview URL ── */
  const previewUrl = slug ? `/haplilist/${slug}` : "/haplilist";
      ...
      <div className="flex gap-4 min-h-[calc(100vh-220px)]">
        ...
        <div className="hidden xl:block xl:w-[60%] shrink-0">
          <div className="sticky top-20">
            <SitePreview url={previewUrl} refreshKey={refreshKey} />
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Input
              ...
              placeholder="פלילי, דעה, מערכת המשפט"
            />
