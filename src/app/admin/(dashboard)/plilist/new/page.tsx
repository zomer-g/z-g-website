...
export default function NewPlilistPostPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/plilist", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.error ?? "שגיאה ביצירת הפוסט. נסו שוב.");
      }
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4 rounded-lg border border-border bg-gray-50/50 p-4">
          <h2 className="text-sm font-semibold text-foreground">הגדרות SEO</h2>
          <Input
            label="כותרת SEO"
            value={seoTitle}
