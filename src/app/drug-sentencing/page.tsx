...
import type { DrugSentencingPageContent } from "@/types/content";
...
export default async function DrugSentencingPage() {
  const [content, session] = await Promise.all([
    getPageContent<DrugSentencingPageContent>("drug-sentencing"),
    auth(),
  ]);
  ...
  return (
    <PublicLayout>
      <EditableSection
        editHref="/admin/site-editor/drug-sentencing"
        editLabel="באנר"
      >
      ...
      <Container className="py-8">
        <RulingsList category="drug-sentencing" legislation={content.legislation} />
      </Container>
    </PublicLayout>
  );
}
